	...
	function requestRss(url){
		...
		$.ajax({
			...
			success: function(xmlDoc){
				var output = "";
				var titleNode = $(xmlDoc).find("channel > title");
				var pubDateNode = $(xmlDoc).find("channel > pubDate");
				
				output += $(titleNode).text()+"<br>";
				...
				$(xmlDoc).find("channel > item").each(function(ind, itemNode){
					...
					output += "<td style='width:350px;'><a href='"+$(itemNode).find("link").text()+'">" + $(itemNode).find("title").text() + "</a></td>";
		...
		<div id="rssHaeder">
			...
			<table class="table">
				<tr>
					...
					<td><a href="javascript:requestRss('${ item.rssLink }')"><c:out value="${ item.title }"/></a></td>
					<%-- <td><a href="jtbc_RSS.jsp?link=${ item.rssLink }"><c:out value="${ item.title }"/></a></td> --%>
					</c:forEach>
				</tr>
			</table>
		</div>
