...
<link rel="canonical"
	href="http://localhost/jsp_pr/common/js/color-modes.js">
<script src="http://localhost/jsp_pr/common/js/bootstrap.min.css"></script>
<meta name="theme-color" content="#712cf9">
<link href="http://localhost/jsp_pr/common/js/carousel.css" rel="stylesheet">
<style>
.bd-placeholder-img {
	font-size: 1.125rem;
	text-anchor: middle;
...
<body>
  ...
  ================================================== -->
		...
		<div class="container marketing">
			...
			<form method="post">
			<!-- form태그에 action속성을저의하지 않으면 submit시 자신으 다시 부른다. -->
			...
			pageContext.setAttribute("nowYear", cal.get(Calendar.YEAR));
			%>
			...
			<div>
				<c:if test="${ not empty param.name }">
					<!-- 웹 파라미터에 name값이 존재하면 선택된 checkbox의 값을 출력 -->
					...
					<table class="table table-hover">
						...
						<tbody>
							<c:if test="${ empty userData }">
								<tr>							
									<td colspan="4" style="text-align: center;">회원의 정보가 존재하지 않습니다.
								</tr>
							</c:if>
							<c:forEach var="userDTO2" items="${ userData }" varStatus="i">
								<c:set var="totalAge" value="${ userDTO2.age + totalAge }"/>
								<c:set var="nowYear" value="${ nowYear }"/>
								<tr>
									<td><c:out value="${ i.count } / ${ userDTO2 }"/></td>
									<td><c:out value="${ userDTO2.myName }"/></td>
									<td><c:out value="${ userDTO2.email }"/></td>
									<td><c:out value="${ userDTO2.age }"/><c:out value=" / ${ nowYear - userDTO2.age }"/></td>
								</tr>
								<c:if test="${ i.last }">
									<tr>							
										<td colspan="3" style="text-align: center;">나이의 합</td>
			...
			<div class="row featurette">
				<div class="col-md-7">
					<div style="padding-left:20px">
						<form action="getParameterFormProcess.jsp" method="GET" id="frm">
	...
	<script src="http://localhost/jsp_pr/common/js/bootstrap.bundle.min.js"
