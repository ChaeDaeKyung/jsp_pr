...
<script src="https://cdn.jsdelivr.net/npm/summernote@0.9.0/dist/summernote-lite.min.js"></script>
	...
	function boardModify( jobFlag ){
		...
		if(confirm("글을 "+msg+" 하시겠습니까?")){
			//폼태그를 얻어서 action 속성을 변경이후 submit 실행
		...
		<div id="divWriteForm" style="margin-top: 20px;">
			...
			if(numArr != null){
		 	...
		 	if(!flag){
				//검색한 글의 카운트를 증가
				bs.modifyCount(num);
				sessionFlag += paramNum+",";
				session.setAttribute("sessionFlag", sessionFlag);
			}// end if
			...
			<form method="post" name="readForm" id="readForm">
				...
				<table>
					...
					<tr>
						<td>제목</td>
						<td>
							<input type="text" name="title" id="title" style="width:400px;" value="${ bDTO.title }">
						</td>
					</tr>
					<tr>
						<td>내용</td>
						<td><textarea name="content" id="content" style="width: 400px; height: 300px;"><c:out value="${ bDTO.content }" escapeXml="true"/></textarea>
					</tr>
					<tr>
						<td>작성자</td>
					...
					<tr>
						<td>작성일</td>
						<td><fmt:formatDate value="${ bDTO.inputDate }" pattern="yyyy-MM-dd"/></td>
					</tr>
					<tr>
